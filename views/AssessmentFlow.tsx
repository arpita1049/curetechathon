...
const AssessmentFlow: React.FC<AssessmentFlowProps> = ({ onBack }) => {
  ...
  const VitalInput = ({ label, icon, value, onChange, unit, color }: any) => (
    <div className="p-4 bg-white/70 dark:bg-[#0f2a47]/60 backdrop-blur-3xl rounded-3xl border border-teal-50 dark:border-white/10 neo-shadow flex items-center gap-4 group transition-all hover:border-teal-200">
      ...
      <div className="flex-grow">
        ...
        <div className="flex items-center gap-2">
          <input
            ...
            className="w-full bg-transparent border-none text-xl font-black text-teal-950 dark:text-white focus:ring-0 p-0"
          />
  ...
  return (
    <div className="animate-slide-up pb-10 max-w-2xl mx-auto">
      <div className="flex items-center justify-between mb-8">
        <button onClick={onBack} className="p-3 bg-white/70 dark:bg-[#0f2a47]/60 backdrop-blur-3xl rounded-2xl text-teal-600 dark:text-teal-400 neo-shadow hover:bg-teal-50 dark:hover:bg-teal-900/30 transition-colors">
          <ChevronLeft />
        </button>
      ...
      <div className="flex items-center justify-between mb-10 px-4">
        {[1, 2, 3].map((s) => (
          <div key={s} className="flex flex-col items-center gap-2 flex-1 relative">
            <div className={`w-10 h-10 rounded-full flex items-center justify-center font-black transition-all border-4 ${step === s ? 'bg-teal-600 text-white border-teal-100 scale-110 neo-shadow' :
            ...
            <div className="p-4 bg-white/70 dark:bg-[#0f2a47]/60 backdrop-blur-3xl rounded-3xl border border-teal-50 dark:border-white/10 neo-shadow flex items-center gap-4 group transition-all hover:border-teal-200">
              ...
              <div className="flex-grow">
                ...
                <select
                  ...
                  className="w-full bg-transparent border-none text-xl font-black text-teal-950 dark:text-white focus:ring-0 p-0 cursor-pointer"
                >
                  <option>Male</option>
      ...
      {step === 2 && (
        <div className="space-y-6">
          <div className="glass bg-white/70 dark:bg-[#0f2a47]/60 backdrop-blur-3xl p-8 rounded-[2.5rem] neo-shadow border border-white/50 dark:border-white/10 space-y-6">
            ...
            <textarea
              className="w-full h-48 p-6 bg-teal-50/30 dark:bg-black/20 border border-teal-100 dark:border-white/10 rounded-3xl focus:ring-4 focus:ring-teal-100/20 outline-none text-teal-950 dark:text-white font-medium text-lg resize-none custom-scrollbar"
          ...
          <div className="flex gap-4">
            <button
              onClick={() => setStep(1)}
              className="flex-1 py-5 bg-white/70 dark:bg-[#0f2a47]/60 backdrop-blur-3xl text-teal-600 dark:text-teal-400 font-black rounded-3xl border border-teal-100 dark:border-white/10 neo-shadow hover:bg-teal-50"
            >
              Back
            </button>
            <button
      ...
      {step === 3 && result && (
        <div className="space-y-6 animate-slide-up">
          <div className="bg-white/70 dark:bg-[#0f2a47]/60 backdrop-blur-3xl p-8 rounded-[3rem] neo-shadow border border-teal-50 dark:border-white/10 space-y-8">
            <div className="flex flex-col md:flex-row justify-between items-start gap-4">
              ...
              <div className={`px-6 py-3 rounded-full text-xs font-black flex items-center gap-3 neo-shadow ${result.riskLevel === RiskLevel.HIGH ? 'bg-red-600 text-white' :
