...
const RiskPrediction: React.FC<RiskPredictionProps> = ({ onBack }) => {
  ...
  return (
    <div className="pb-10 animate-fade-in">
      <button onClick={onBack} className="mb-6 flex items-center text-teal-600 dark:text-teal-400 font-bold px-4 py-2 bg-white/30 dark:bg-[#0f2a47]/60 backdrop-blur-md border border-slate-200/50 dark:border-white/10 rounded-2xl">
      ...
      {!risks ? (
        <div className="space-y-6">
          <div className="bg-white/70 dark:bg-[#0f2a47]/60 backdrop-blur-3xl p-6 rounded-3xl border border-gray-100 dark:border-white/10 shadow-sm">
            <label className="block text-sm font-bold text-gray-700 dark:text-white/70 mb-3 uppercase tracking-wider">Lifestyle Factors</label>
            <textarea
              className="w-full h-40 p-4 bg-gray-50 dark:bg-black/20 border border-gray-200 dark:border-white/10 rounded-2xl focus:ring-2 focus:ring-teal-500 outline-none text-gray-900 dark:text-white"
        ...
        <div className="space-y-6 animate-scale-in">
          {risks.map((risk, idx) => (
            <div key={idx} className="bg-white/70 dark:bg-[#0f2a47]/60 backdrop-blur-3xl p-6 rounded-3xl border border-gray-100 dark:border-white/10 shadow-md">
              <div className="flex justify-between items-center mb-4">
                <h3 className="text-xl font-bold text-gray-800 flex items-center gap-2">
                  <Activity className="text-teal-500" /> {risk.disease}
                </h3>
                <div className={`px-4 py-1 rounded-full text-xs font-bold ${risk.meterLevel === RiskLevel.HIGH ? 'bg-red-100 text-red-600' :
              ...
              <div className="mb-6">
                ...
                <div className="h-4 w-full bg-gray-100 rounded-full overflow-hidden flex">
                  <div
                    className={`h-full transition-all duration-1000 ${risk.riskPercentage > 70 ? 'bg-red-500' : risk.riskPercentage > 40 ? 'bg-orange-400' : 'bg-green-500'
                      }`}
